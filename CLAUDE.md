...
- **No deploy job.** Cloudflare Pages auto-deploy is disabled for this repository, because the placeholder ids in `wrangler.toml` make a production build fail by design (see `docs/CLOUDFLARE_SETUP.md`). Production ships by hand: `bun run deploy` from a checkout whose `wrangler.toml` carries the real ids. **The Pages project is `starspace-website`, not `starspace-group`** — the apex domain `starspace.group` is bound to the former; the latter exists, is domainless, and silently accepted deploys until `wrangler.toml`'s `name` was corrected on 2026-09-09. Its production bindings match the ids in a set-up `wrangler.toml` (D1 `e8fe6b0e…`, KV `d2baf6f5…`, R2 `starspace-group-files`), so a direct upload does not repoint prod at other resources. A push to `main` alone deploys nothing.

This describes the workflow's _intended_ gates. Per AGENTS.md's Verification rule, don't report them as passing without observing a run. The repository was published to `starspacegroup/website` on 2026-09-06; the runs below predate that and were observed on `starspacegroup/starspace-group-nebulakit`, which no longer exists. Treat CI on the new remote as unobserved until a run is seen there. The workflow had observed runs: both jobs passed on PR #7's head (2026-08-08, run `31262364171`) and on PR #9's head `8443b8b` (2026-08-10, run `31439300377`). Before 2026-08-08 it had never executed — the fork-PR approval gate parked fork-PR runs at `action_required` with zero jobs, and that gate still applies to each new fork-PR push until a maintainer approves the run. The canary-lockfile trap above is not hypothetical: PR #6's head still carries a `lockfileVersion: 2` `bun.lock` and fails both jobs at install (run `31434971833`); the regenerated lockfile landed on PR #9's branch as `e3b87b9`.
...
**Connecting SpaceBot is one click when the site is registered with it.** `src/lib/server/spacebot-connect.ts` plus `/admin/spacebot/connect` and `/admin/spacebot/callback` implement the client half of SpaceBot's Connect handshake: the owner is sent to SpaceBot's consent screen, picks a server, approves `voice:read` and `stats:read`, and the site trades the returned one-time code for a key **server to server**. What crosses the browser is a code, useless without `SPACEBOT_CONNECT_CLIENT_SECRET`, so the key never appears in a URL, a referrer header or a log. `state` is generated before the redirect, kept in an httpOnly `sameSite: 'lax'` cookie scoped to `/admin/spacebot`, compared in constant time on return and cleared whatever happens — without it a crafted link could connect the site to somebody else's SpaceBot. The button only renders when `readConnectClient` finds a complete registration; the paste-a-key form stays as the fallback and as the whole page otherwise, and `?connect=` outcomes map to fixed copy in the page loader rather than round-tripping an error string into the admin UI.

**The home hero's voice panel reads SpaceBot, and fails to a simulation.** `src/lib/server/voice-channel.ts` calls SpaceBot's `GET /api/v1/voice` with an `sb_live_…` key carrying the `voice:read` scope (`SPACEBOT_API_URL`, `SPACEBOT_API_KEY`, `SPACEBOT_VOICE_CHANNEL`), and `src/routes/api/voice/+server.ts` is the public, KV-cached proxy the browser polls; `VoiceChannel.svelte` picks between `VoiceChannelLive` and the CSS-animated `VoiceChannelDemo`. Two properties are load-bearing:
...
**The server guide publishes what SpaceBot measured, and nothing else.** `/guide` server-renders from `src/lib/server/guild-directory.ts`, which reads `GET /api/v1/channels?activity=30` and `GET /api/v1/commands` with the same key (`channels:read`, `commands:read`) and caches the result in KV for a day. `?activity=30` is what makes the page worth reading: a channel's name and a topic somebody set once cannot say which rooms are alive, and a voice channel usually has no topic at all, so SpaceBot returns per-channel counts — messages and distinct posters with the time of the last one, voice seconds, head count, visit count, average visit length, the hour it fills up, and whether joining it spawns a room. Counts only; no message text or member name is ever sent. `src/lib/channel-activity.ts` turns them into the sentence under each channel and holds the rules worth not breaking: a channel in the server's `excluded_channels` reports `null` and renders as "not counted" rather than as quiet, an absent field is never printed as a confident zero, and a SpaceBot too old to know `?activity` leaves every channel rendering exactly as it did before, and a forum or media channel gets no line at all because Discord logs its messages against their threads, so its own count is structurally zero however busy it is. The window is labelled from `activity_days` as the API answered it, not as the site asked.

**`/stats` publishes the server's figures, and shows a member their own.** Two halves that fail apart. The public half is `src/lib/server/guild-stats.ts` reading `GET /api/v1/stats` with the `stats:read` the site already holds, cached in KV as `guild:stats` for ten minutes — aggregates only, shown to everyone, no sign-in wall in front of it. The personal half is `src/lib/server/member-profile.ts` reading `GET /api/v1/members/:id` behind a **new `members:read` scope**, which `stats:read` deliberately does not imply and which no key issued before 2026-09-22 carries. Three properties are load-bearing:

- **The Discord id comes from `oauth_accounts` for `locals.user.id`, and from nowhere else.** `discordAccount` is the only source, and it re-checks the stored value against `^\d{5,32}$` because it goes into a URL. It carries the avatar hash from that same row, which is why the panel can show the member their own picture without SpaceBot ever sending one. SpaceBot cannot tell whether the asker owns the account — a key with `members:read` may ask about any member — so a `?user=` parameter on this page would turn it into a lookup service. That check is entirely this site's to keep.
- **One person's figures are never in KV, and the page is `private, no-store`.** Only `guild:stats` is cached, because it is the same answer for every visitor. `clearSpaceBotConnection` drops it alongside the voice and history entries.
- **Every failure lands one state up, never on an error.** No key, no `members:read`, an unreachable bot: the public page renders and the panel says so. A snapshot with no member count renders the daily rows anyway. See `docs/PUBLIC_STATS.md` for the numbers the page deliberately refuses to compute.

Presentation lives in `src/lib/stats-copy.ts` rather than beside the fetcher, because a component imports it and SvelteKit will not let client code reach into `$lib/server` — the same split `member-stats.ts` keeps from `member-history.ts`.
...
`migrations/` is D1-native — wrangler sorts `.sql` files by numeric prefix and then by string, and records applied ones in `d1_migrations` **by filename**. A filename already on `main` is already applied and is immutable (AGENTS.md §2, `migrations/README.md`). The sequence runs `0001_`–`0015_`, the next is `0016_`.
...
| Public `/stats`, the personal panel, `members:read` | `PUBLIC_STATS.md`                                           |
...
- **The public content is checked-in data.** `src/lib/data/projects.ts` backs `/projects`; `featuredProjects` is the head of the same list, so the home shelf and the directory cannot drift. Artwork lives in `static/projects/` as ~900px WebP, and `tests/unit/site-content.test.ts` fails when an entry names a file that is not on disk. `bun run capture:projects [ids…]` re-screenshots the projects' live sites into `static/projects/`; it reads the ids from the same data module, and it deliberately does **not** touch the descriptions — a redesign usually makes the copy stale too, and that needs reading the page. The Discord invite code lives once, in `src/lib/discord.ts`. Deliberately not CMS-backed: the page has to render on a clone that has never been pointed at D1. A `/sister-spaces` route existed and was parked on 2026-09-04 — see `tasks/todo.md`; restore it from git rather than rewriting it.
