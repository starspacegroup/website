...
- **The three-person threshold is server-side.** Below `LIVE_MEMBER_THRESHOLD` the module returns `{ live: false }` with no member data — not a count, not a trimmed list. Who is sitting in a two-person voice channel is a fact about those two people, and moving that check into the component would put their names in the browser.
